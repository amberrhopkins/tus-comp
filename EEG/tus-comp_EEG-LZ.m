...
eeglab nogui
%% make save folder
mkdir([main_dir '/LZ_results']);
savepath=[main_dir '/LZ_results'];

%%
addpath([main_dir '/Daniel']); %LZ function

%% THIS CAN BE MADE SMARTER. CAN COMPUTE LZ JUST FOR THE SETs it did not compute yet

if isfile([savepath '/data_LZ_10ms.mat']) %skip if already computed
    load([savepath '/data_LZ_10ms.mat'])
else
    data_LZ=dir([main_dir '/derivatives/**/*rseeg*Final*.set']);

    for x=1:length(data_LZ)
        EEG=pop_loadset(data_LZ(x).name,...
            data_LZ(x).folder);
        ...
        % LZ calc
        data_LZ(x).LZ_10ms=fJLZC(mean(EEG.data,3), EEG.srate,[]);

        disp(['***dataset ' data_LZ(x).name ' DONE***' ]);

        %     else
        %         data_LZ(x).LZ_7ms=NaN;
        %     disp(['***dataset ' data_LZ(x).name ' TEP too small!!  SKIPPING ***' ]);
        %     end

    end
    save([savepath '/data_LZ_10ms'],...
        'data_LZ');
end
...
for x=1:length(data_LZ)
    % find sub number
    subnum=str2double(char(extractBetween(data_LZ(x).name, 5,7)));
    %find ses number
    sesnum=str2double(char(extractBetween(data_LZ(x).name, 13,15)));
    ...
    %store target
    data_LZ(x).target=char(order_condition_sub_ses.target);
    data_LZ(x).target_num=order_condition_sub_ses.ses;
    data_LZ(x).sub_num=order_condition_sub_ses.subNum;
end
...
%subject list
subj_list=strtok({data_LZ.name}, '_');
subj_list=unique(subj_list);

for sub_count=1:length(subj_list)
    current_sub= data_LZ(startsWith({data_LZ.name}, subj_list{sub_count})); %isolate one subject
    ...
    for cond_all_count=1:length(current_sub) %plot that subjext, one plot per condition
        if strcmp(current_sub(cond_all_count).target, 'TAL')
            ...
            plot(1:length(current_sub_T_ordered), [current_sub_T_ordered(1:end).LZ_10ms], '-x')
            text(1:length(current_sub_T_ordered), [current_sub_T_ordered(1:end).LZ_10ms], strtok({current_sub_T_ordered(1:end).name}, '_'),'VerticalAlignment','bottom','HorizontalAlignment','right')
        ...
        elseif strcmp(current_sub(cond_all_count).target, 'GPE')
            ...
            plot(1:length(current_sub_G_ordered), [current_sub_G_ordered(1:end).LZ_10ms], '-x')
            text(1:length(current_sub_G_ordered), [current_sub_G_ordered(1:end).LZ_10ms], strtok({current_sub_G_ordered(1:end).name}, '_'),'VerticalAlignment','bottom','HorizontalAlignment','right')
        ...
        elseif strcmp(current_sub(cond_all_count).target, 'SHA')
            ...
            plot(1:length(current_sub_S_ordered), [current_sub_S_ordered(1:end).LZ_10ms], '-x')
            text(1:length(current_sub_S_ordered), [current_sub_S_ordered(1:end).LZ_10ms], strtok({current_sub_S_ordered(1:end).name}, '_'),'VerticalAlignment','bottom','HorizontalAlignment','right')
